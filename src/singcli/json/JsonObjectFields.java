...
public final class JsonObjectFields {
    private JsonObjectFields() {
    }

    public static FieldLocation findField(String objectJson, String field) {
        int objectStart = JsonSyntax.skipWhitespace(objectJson, 0);
        ...
        for (int i = objectStart; i < objectJson.length(); i++) {
            ...
            if (ch == '"') {
                JsonSyntax.ParsedString parsed = JsonSyntax.parseString(objectJson, i);
                if (objectDepth == 1 && arrayDepth == 0) {
                    int colon = JsonSyntax.skipWhitespace(objectJson, parsed.end() + 1);
                    if (colon < objectJson.length() && objectJson.charAt(colon) == ':'
                            && field.equals(parsed.value())) {
                        int valueStart = JsonSyntax.skipWhitespace(objectJson, colon + 1);
                        int valueEnd = JsonSyntax.findValueEnd(objectJson, valueStart);
    ...
    public static String addField(String objectJson, String field, String value, String indent) {
        ...
        int objectEnd = JsonSyntax.findMatchingObjectEnd(objectJson, objectStart);
        boolean empty = JsonSyntax.skipWhitespace(objectJson, objectStart + 1) == objectEnd;
    ...
    public static List<String> stringArrayField(String objectJson, String field) {
        ...
        int arrayEnd = JsonSyntax.findMatchingArrayEnd(objectJson, location.valueStart());
        for (int i = location.valueStart() + 1; i < arrayEnd; i++) {
            i = JsonSyntax.skipWhitespace(objectJson, i);
            if (i >= arrayEnd) {
                break;
            }
    ...
    public static String stringField(String objectJson, String field, String fallback) {
        ...
        return JsonSyntax.parseString(objectJson, location.valueStart()).value();
    }
